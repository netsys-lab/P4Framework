    ...
    inout standard_metadata_t smeta)
{
    ...
    action sendToCPUNoPath() {
        ...
        hdr.cpu.setValid();
        hdr.cpu = { TO_CPU_REASON_NO_PATH, 0 };
    ...
    table tab_classifier_stage1 {
        ...
        const default_action = set_traffic_class(0);
        direct_match = true;
    }
    ...
    // Single bit counters that are set whenever the corresponding path has been
    // hit.
    Counter<bit<1>, PathIndex_t>(PATH_TABLE_SIZE, CounterType_t.PACKETS) cnt_path_hit;
